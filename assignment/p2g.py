...
def p2g(state: StateStruct, model: ModelStruct):
    ...
    PURPOSE:
    This function transfers particle mass and velocity to the grid using a cubic B-spline
    interpolation kernel. Each particle distributes its mass and velocity to a 3×3×3
    ...
    - grid_v_in[ix, iy, iz]: vec3 - Grid velocity array (accumulated via atomic_add)
        Each grid cell accumulates velocity from all particles in its neighborhood.
        After all particles are processed, grid_v_in[i,j,k] contains the total velocity at grid cell (i,j,k)
